...
AttributeComponent::AttributeComponent(unsigned level)
{
    this->level = level;
    this->exp = 0;
    this->expNext = static_cast<unsigned>((50/3) * (std::pow(this->level + 1, 3) - 6 * pow(this->level + 1, 2) + ((this->level+1) * 17) - 12));
...
void AttributeComponent::levelUp()
{
    while(this->exp >= this->expNext)
    {
        ...
        this->expNext = static_cast<unsigned>((50/3) * (std::pow(this->level, 3) - 6 * pow(this->level, 2) + (this->level * 17) - 12));
