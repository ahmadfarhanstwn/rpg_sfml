...
{
    this->animations[key] = new Animation(this->sprite, this->textureSheet, animation_timer, start_frame_x,
                                          start_frame_y, frames_x, frames_y, width, height);
}

const bool& AnimationComponent::play(const std::string key, const float& dt, const bool priority)
{
    if (this->priorityAnimation) //play priority animation
    {
        if (this->priorityAnimation == this->animations[key])
        {
            ...
            //If the animation is done, remove the priority
    ...
    {
        //if there is priority, set the priority animation to the key
        if(priority)
        {
...
const bool& AnimationComponent::play(const std::string key, const float& dt, const float& modifier, const float& modifier_max, const bool priority)
{
    if (this->priorityAnimation) //play priority animation
    {
        if (this->priorityAnimation == this->animations[key])
        {
            ...
            //If the animation is done, remove the priority
            if(this->animations[key]->play(dt, (modifier/modifier_max)))
            {
                this->priorityAnimation = NULL;
            }
        }
